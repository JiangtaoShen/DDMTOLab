"""
Many-objective Multifactorial Evolutionary Algorithm With Online Transfer Parameter Estimation (MaO-MFEA-II)

This module implements MaO-MFEA-II for many-objective multi-task optimization with knowledge transfer.
...
----------
    ...
    [2] Y. Tian, R. Cheng, X. Zhang, Y. Su, and Y. Jin. A strengthened dominance relation considering \
...
from Algorithms.STMO.NSGAIISDR import nsga2sdr_sort
from Methods.Algo_Methods.algo_utils import *


class MaOMFEAII:
    """
    Many-objective Multifactorial Evolutionary Algorithm With Online Transfer Parameter Estimation.
    ...
    def __init__(self, problem, n=None, max_nfes=None, save_data=True, save_path='./TestData',
                 name='maomfea2_test', disable_tqdm=True):
        """
        Initialize MaO-MFEA-II.
        ...
        name : str, optional
            Name for the experiment (default: 'maomfea2_test')
    ...
    def optimize(self):
        """
        Execute the MaO-MFEA-II algorithm.
        ...
        while nfes < max_nfes:
            ...
            # Perform NSGA-II-SDR sorting to get dominance ranks for each task
            rank = []
            for i in range(nt):
            ...
            pop_decs, objs, cons, pop_sfs = maomfea_selection(pop_decs, pop_objs, pop_cons, pop_sfs, n, nt)
...
def maomfea_selection(all_decs, all_objs, all_cons, all_sfs, n, nt):
    """
    Environmental selection for MaO-MFEA-II using constraint-first and NSGA-II-SDR criteria.
    ...
    all_cons : np.ndarray
        Constraint violation matrix corresponding to all_decs of shape (n_total, n_con)
    all_sfs : np.ndarray
    ...
    for i in range(nt):
        ...
        # Calculate total constraint violation for each individual
        # Constraint violation is the sum of positive constraint values (violations)
        cv = np.sum(np.maximum(0, current_cons), axis=1)

        # NSGA-II-SDR sorting: rank based on strengthened dominance relation and crowding distance
        sdr_rank, _, _ = nsga2sdr_sort(current_objs)

        # Combined sorting: constraint violation first, then SDR rank
        # Use lexsort: primary key is cv (ascending), secondary key is sdr_rank (ascending)
        # lexsort sorts by last key first, so order is (sdr_rank, cv) for cv-first sorting
        sorted_indices = np.lexsort((sdr_rank, cv))

        # Select top-n individuals
        indices_select = sorted_indices[:n]
