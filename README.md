...
- **2026-08-21** — A task can now declare each variable's type — `F` float, `I` integer, `B` binary, `O` ordinal, `C` categorical — with `var_values` naming what a categorical or ordinal index stands for. Descriptive only, so existing problems evaluate unchanged.
- **2026-08-21** — Cross-checked every benchmark against PlatEMO and MToP. **MW results from before this date are not valid**: all fourteen were wrong by up to 100% of their range. WFG1 and the DTLZ8 and ZDT3 reference fronts were also fixed; the rest already matched to machine precision. MTMO-DTLZ dropped: 28 suites, 211 problems.
